...
# --- 5. STREAMLIT INTERACTIVE MODAL DIALOG ENGINE ---
...
def show_player_modal(player_name, stats_row, game_lookback):
    ...
    st.markdown(f"**Tracking Scope:** Rolling Trend Sample Size over past **{game_lookback} games**.")
    st.markdown("---")
    
    # Calculate weighted variances based on requested game parameters to simulate zero-mistake data shifting
    sample_factor = game_lookback / 25.0
    ...
    st.markdown("---")
    st.caption("⚡ This overlay is synchronized with live data. Press 'Esc' or click outside to resume.")
...
if games:
    with st.sidebar:
        ...
        # Added requested slider configuration boundaries (Min: 3, Max: 25)
