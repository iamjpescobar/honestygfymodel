...
wind           - NOT here. engines/player_of_the_day._wind_hr_adj already
                 handles it correctly and honestly: it adjusts only on
                 MLB's field-relative string ("12 mph, Out To CF") and
                 returns 0 on a compass forecast ("SW 12 mph"), because
                 southwest tells you nothing without knowing which way
                 the stadium faces.

                 Resolving compass winds needs each park's home-plate-to-
                 centre-field bearing. That's a 30-row table of real
                 surveyed values, and inventing it from memory would put
                 confident numbers behind nothing — the same mistake that
                 produced the inverted K Score. It needs a verified
                 source, so wind stays as it is until then.
"""
...
def context_hr_adj(home_team_abbr, bats, temp_str=None, roof_closed=False):
    """Combined park + temperature adjustment for one batter tonight.
    ...
    notes = [n for n in (p_note, t_note) if n]
    return round(p_adj + t_adj, 1), notes
