...
# WHICH UNIT THE CAP COUNTS. "game" allows GAME_CAP bats from a matchup;
# "team" allows GAME_CAP from EACH SIDE, so up to twice as many from one
# building.
#
# This board was built with unit="game" and cap 2 to answer a specific
# complaint: one lineup taking over the top fifteen. Park factor,
# temperature, wind and the opposing arsenal lift BOTH sides at once, so
# the game is the unit that correlation actually travels on.
#
# Moved to "team" at 3 deliberately, for more room at the top. The
# trade is real and worth restating rather than discovering: on a
# hitter's-park night this permits SIX bats from one game where the old
# rule permitted two. If the board starts stacking again, this constant
# and CAP_UNIT are the whole fix.
CAP_UNIT = "team"


def cap_per_game(rows, cap=GAME_CAP, unit=None):
