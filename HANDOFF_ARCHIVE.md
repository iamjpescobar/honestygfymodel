# Los Cappers — handoff archive

Entries rotated out of `HANDOFF.md` to keep the working document
readable. **Nothing here is current.** Every entry describes a defect
that has since been fixed, a threshold since re-measured, or a design
since replaced — several of them more than once.

Kept because the REASONING is occasionally worth re-reading, not because
the state is accurate. For what is true now, read `HANDOFF.md`.
