...
def fetch_team_roster(team_id, debug=False):
    ...
    THE EXTRA FIELDS ARE NOT NEW REQUESTS. They were in this response all
    along and were being dropped on the floor: the old version kept
    {name, pos} and discarded jersey, status, injuries[], experience and
    the rest. That is why the app's Status column had to INFER "OUT 4d"
    from days-since-played — not because the fact was unavailable, but
    because it was thrown away one function earlier.

    Field coverage measured on the 2026-08-03 slate, six teams:
    id, displayName, position, status, experience, headshot, age and
    height on 14 of 14 athletes; jersey on 13 of 14; injuries[] on the
    1-4 per team who have one. `active` and `starter` do NOT exist on
    this payload at all — do not look for them here.
