#!/usr/bin/env python3
"""Re-applies the Home.py half of the cross-sport cards + card restyle.

app.py already has its lc_pending_sport consumer; this file does not touch it.
...
# Edits ACCUMULATE per file. The previous version of this script re-read
# each file from disk per edit and wrote them all at the end, so on a
# file with several edits only the last one survived - and it still
# printed "patched" for every one of them.
buf = {}
applied = []
...
# Verify what is ON DISK, not what we meant to write.
_home = (ROOT / "app/views/Home.py").read_text()
_checks = {
    "_goto_sport defined": "def _goto_sport(" in _home,
    "writes lc_pending_sport": 'st.session_state["lc_pending_sport"]' in _home,
    "cross-sport cards": "home_other_" in _home,
    "edge variable": "--lc-edge" in _home,
    "sport badge": "lc-elsewhere" in _home,
    "no lc_sport_seg write": "lc_sport_seg" not in
        _home.split("CURRENT_SPORT")[-1].replace(
            'st.session_state.get("lc_sport_seg")', ""),
}
print()
for name, ok in _checks.items():
    print(f"  {'OK  ' if ok else 'FAIL'} {name}")
print("done" if all(_checks.values()) else "INCOMPLETE - tell Claude")
